...
const track = async (
  ...
  const rangeStartTime = getRangeStartTime(
    rangeType as 'year' | 'month' | 'day' | 'hour'
  )
...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (req.method === 'OPTIONS') {
    ...
    if (referrer == null || referrer == undefined)
      return res.status(400).send('No referrer specified')
    ...
    if (
      website.get('domain') !== normalizeDomain(url.hostname) &&
      url.hostname !== 'localhost'
    ) {
      return res
        .status(400)
        .send(`Wrong domain idiot, expected ${website.get('domain')}`)
    }
